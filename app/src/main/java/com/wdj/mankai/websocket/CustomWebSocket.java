package com.wdj.mankai.websocket;
...
import com.wdj.mankai.data.model.JsonModel;
import com.wdj.mankai.observers.CustomSdpObserver;
import com.wdj.mankai.openvidu.LocalParticipant;
import com.wdj.mankai.openvidu.Participant;
...
public class CustomWebSocket extends AsyncTask<SessionActivity, Void, Void> implements WebSocketListener {
    ...
    private void handleServerResponse(JSONObject json) throws
            JSONException {
        final int rpcId = json.getInt(JsonModel.ID);
        JSONObject result = new JSONObject(json.getString(JsonModel.RESULT));
        ...
        if (result.has("value") && result.getString("value").equals("pong")) {
            ...
            final String localConnectionId = result.getString(JsonModel.ID);
            this.mediaServer = result.getString(JsonModel.MEDIA_SERVER);
            ...
            if (result.getJSONArray(JsonModel.VALUE).length() > 0) {
    ...
    public void joinRoom() {
        ...
        joinRoomParams.put(JsonModel.METADATA, "{\"clientData\": \"" + this.session.getLocalParticipant().getParticipantName() + "\"}");
        ...
        this.ID_JOINROOM.set(this.sendJson(JsonModel.JOINROOM_METHOD, joinRoomParams));
    }

    public void leaveRoom() {
        this.ID_LEAVEROOM.set(this.sendJson(JsonModel.LEAVEROOM_METHOD));
    }
    ...
    public void publishVideo(SessionDescription sessionDescription) {
        ...
        this.ID_PUBLISHVIDEO.set(this.sendJson(JsonModel.PUBLISHVIDEO_METHOD, publishVideoParams));
    }
    ...
    public void prepareReceiveVideoFrom(RemoteParticipant remoteParticipant, String streamId) {
        ...
        this.IDS_PREPARERECEIVEVIDEO.put(this.sendJson(JsonModel.PREPARERECEIVEVIDEO_METHOD, prepareReceiveVideoFromParams), new Pair<>(remoteParticipant.getConnectionId(), streamId));
    }
    ...
    public void receiveVideoFrom(SessionDescription sessionDescription, RemoteParticipant remoteParticipant, String streamId) {
        ...
        this.IDS_RECEIVEVIDEO.put(this.sendJson(JsonModel.RECEIVEVIDEO_METHOD, receiveVideoFromParams), remoteParticipant.getConnectionId());
    }
    ...
    public void onIceCandidate(IceCandidate iceCandidate, String endpointName) {
        ...
        this.IDS_ONICECANDIDATE.add(this.sendJson(JsonModel.ONICECANDIDATE_METHOD, onIceCandidateParams));
    }

    private void handleServerEvent(JSONObject json) throws JSONException {
        if (!json.has(JsonModel.PARAMS)) {
            Log.e(TAG, "No params " + json.toString());
        } else {
            final JSONObject params = new JSONObject(json.getString(JsonModel.PARAMS));
            String method = json.getString(JsonModel.METHOD);
            switch (method) {
                case JsonModel.ICE_CANDIDATE:
                    iceCandidateEvent(params);
                    break;
                case JsonModel.PARTICIPANT_JOINED:
                    participantJoinedEvent(params);
                    break;
                case JsonModel.PARTICIPANT_PUBLISHED:
                    participantPublishedEvent(params);
                    break;
                case JsonModel.PARTICIPANT_LEFT:
                    participantLeftEvent(params);
                    break;
                case JsonModel.SEND_MESSAGE:
                    Log.e(TAG, "Send Message " + json.toString());

                    break;
                case JsonModel.STREAM_PROPERTY_CHANGED:
                    Log.e(TAG, "StreamChanged " + json.toString());
                    break;
                default:
    ...
    public synchronized int sendJson(String method, Map<String, String> params) {
        ...
        try {
            ...
            jsonObject.put("jsonrpc", JsonModel.JSON_RPCVERSION);
    ...
    private void addRemoteParticipantsAlreadyInRoom(JSONObject result) throws
            JSONException {
        for (int i = 0; i < result.getJSONArray(JsonModel.VALUE).length(); i++) {
            JSONObject participantJson = result.getJSONArray(JsonModel.VALUE).getJSONObject(i);
    ...
    private void participantPublishedEvent(JSONObject params) throws
            JSONException {
        String remoteParticipantId = params.getString(JsonModel.ID);
    ...
    private RemoteParticipant newRemoteParticipantAux(JSONObject participantJson) throws JSONException {
        final String connectionId = participantJson.getString(JsonModel.ID);
        String participantName = "";
        if (participantJson.getString(JsonModel.METADATA) != null) {
            String jsonStringified = participantJson.getString(JsonModel.METADATA);
            try {
    ...
    private void pingMessageHandler() {
        ...
        executor.scheduleWithFixedDelay(() -> {
            ...
            ID_PING.set(sendJson(JsonModel.PING_METHOD, pingParams));
